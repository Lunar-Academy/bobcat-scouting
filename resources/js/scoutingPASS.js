...
function addTimer(table, idx, name, data) {
  ...
  button1.setAttribute("class", "start");
...
// inp.setAttribute("onchange", "checkzero();");
  ...
  button2.setAttribute("class", "reset");
...
function addText(table, idx, name, data) {
  ...
  inp.setAttribute("onchange", "replaceSemicolon(this.value)");
  if (enableGoogleSheets && data.hasOwnProperty('gsCol')) {
    inp.setAttribute("name", data.gsCol);
  } else {
    inp.setAttribute("name", data.code);
  ...
  if (data.hasOwnProperty('defaultValue')) {
    ...
    def.setAttribute("type", "hidden");
    def.setAttribute("value", data.defaultValue);
    def.setAttribute("onchange", "replaceSemicolon(this.value)");
    cell2.appendChild(def);
  }
...
function addRadio(table, idx, name, data) {
  ...
  var cell2 = row.insertCell(1);
  cell2.setAttribute("id", "input1_" + data.code);
  ...
  if (data.hasOwnProperty('choices')) {
    ...
    keys.forEach(c => {
      ...
      inp.setAttribute("classname", "radio");
      ...
      // cell2.appendChild(frag)

      cell2.appendChild(inp);

      var name= document.createElement("p");
      name.setAttribute("class", "radioelement");
      cell2.appendChild(name);

      name.innerHTML += data.choices[c];
...
function addArray(table, idx, name, data) {
...
cell2.appendChild(tbl);
      ...
      //   for(let b=0; b<3; b++){
         ...
         var cell3 = tbl.rows[1].cells[1];
         if (data.hasOwnProperty('lowcone')) {
          keys = Object.keys(data.lowcone);
          keys.forEach(c => {
         ...
         cell3.innerHTML += data.lowcone[c];

        });
      }
    
      var cell3 = tbl.rows[1].cells[2];
      if (data.hasOwnProperty('lowcube')) {
       keys = Object.keys(data.lowcube);
       keys.forEach(c => {
      ...
      cell3.innerHTML += data.lowcube[c];

     });
   }

   var cell3 = tbl.rows[2].cells[1];
   if (data.hasOwnProperty('midcone')) {
   ...
   cell3.innerHTML += data.midcone[c];

  });
}

var cell3 = tbl.rows[2].cells[2];
if (data.hasOwnProperty('midcube')) {
 keys = Object.keys(data.midcube);
 keys.forEach(c => {
...
cell3.innerHTML += data.midcube[c];

});
}

var cell3 = tbl.rows[3].cells[1];
if (data.hasOwnProperty('highcone')) {
 keys = Object.keys(data.highcone);
 keys.forEach(c => {
...
cell3.innerHTML += data.highcone[c];

});
}

var cell3 = tbl.rows[3].cells[2];
if (data.hasOwnProperty('highcube')) {
...
cell3.innerHTML += data.highcube[c];
...
function addElement(table, idx, data) {
  ...
  if (type == 'counter') {
    ...
    idx = addArray(table, idx, name, data);
  } 
  
  else if 
  (data.type == 'comments') {
idx = addComments(table, idx, name, data);
}
...
function swipePage(increment) {
  if (qr_regenerate() == true) {
    ...
    if (slide + increment < slides.length && slide + increment >= 0) {
      ...
      // $( ".radio:visible" ).wrap( "<label></label>" );
...
function addComments(table, idx, name, data) {
  var row = table.insertRow(idx);
  var cell1 = row.insertCell(0);
  cell1.classList.add("title");
  if (!data.hasOwnProperty('code')) {
